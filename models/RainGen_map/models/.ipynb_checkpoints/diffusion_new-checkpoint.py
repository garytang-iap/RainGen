...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            # 修改后的代码
            img = torch.randn(*shape, device=device, dtype=next(model.parameters()).dtype)
        ...
        for i in indices:
            ...
            with torch.no_grad():
                def cfg_model_fn(x, t_scaled):
                    if guidance_scale == 1.0:
                        return model(x, t_scaled, **model_kwargs, cond_drop_prob=0.0)
                    
                    pred_cond = model(x, t_scaled, **model_kwargs, cond_drop_prob=0.0)
                    pred_uncond = model(x, t_scaled, **model_kwargs, cond_drop_prob=1.0)
    ...
    def _knn_interpolate_to_grid(self, x, sample_lst, img_size, max_img_size):
        """
        现在这个方法直接调用我们自己实现的、不依赖pytorch3d的KNN插值函数。
        """
        # 1. 准备稀疏点的坐标
        # 我们需要从 sample_lst（索引）中恢复出原始的归一化坐标
        coords_grid = torch.stack(torch.meshgrid(
            torch.linspace(0, 1, steps=img_size, device=x.device),
            torch.linspace(0, 1, steps=img_size, device=x.device),
            indexing="ij"
        )) # Shape: [2, H, W]
        coords_grid_flat = rearrange(coords_grid, 'c h w -> () (h w) c') # Shape: [1, S_full, 2]
        
        # 扩展以匹配 batch size
        coords_grid_flat = coords_grid_flat.repeat(x.size(0), 1, 1)
        
        # 使用 sample_lst gather出我们需要的稀疏坐标
        sparse_coords = torch.gather(
            coords_grid_flat, 
            1, 
            sample_lst.unsqueeze(2).expand(-1, -1, 2)
        ).contiguous() # Shape: [B, S, 2]
        
        # 2. 准备目标网格的坐标
        out_coords_grid = torch.stack(torch.meshgrid(
            torch.linspace(0, 1, steps=max_img_size, device=x.device),
            torch.linspace(0, 1, steps=max_img_size, device=x.device),
            indexing="ij"
        )) # Shape: [2, H_out, W_out]
        out_coords_flat = rearrange(out_coords_grid, 'c h w -> () (h w) c') # Shape: [1, N_full, 2]
        out_coords_flat = out_coords_flat.repeat(x.size(0), 1, 1) # Match batch size

        # 3. 调用我们自己写的插值函数
        # x 的形状需要是 [B, S, D]，而它现在是 [B, D, S] 或类似，需要确认
        # 你的 _gather_from_grid 返回的是 [B, S, C]，所以 x 的形状是正确的
        interpolated_features = knn_interpolate(
            features=x,
            coords=sparse_coords,
            grid_coords=out_coords_flat,
            k=3 # 使用一个合理的默认值，或者从配置中读取
        )
        
        # 4. 重塑回图像格式
        # interpolated_features 的形状是 [B, N, D]
        out = rearrange(
            interpolated_features, 
            "b (h w) c -> b c h w", 
            h=max_img_size, w=max_img_size
        )
