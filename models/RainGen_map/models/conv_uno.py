...
FLASH_ATTN_V2_AVAILABLE = True
# 在 conv_uno.py 中，替换掉原来的 FlashAttnBlock

# in conv_uno.py
import warnings
class FlashAttnBlock(nn.Module):
    """
    一个使用 PyTorch 标准 nn.MultiheadAttention 的注意力块。
    强制回退到标准实现，以避免 flash-attn 的复杂兼容性问题。
    """
    def __init__(self, dim, min_heads=4, dim_head=64, dropout=0.1, **kwargs):
        super().__init__()

        self.head_dim = dim_head
        self.num_heads = dim // self.head_dim

        if dim % self.head_dim != 0:
            # 如果维度不可整除，进行调整并发出警告
            original_num_heads = self.num_heads
            for h in range(original_num_heads, 0, -1):
                if dim % h == 0:
                    self.num_heads = h
                    break
            if self.num_heads == 0: self.num_heads = 1 # 确保至少有一个头
            warnings.warn(f"Warning: Attention dim {dim} is not divisible by head_dim {dim_head}. Adjusted num_heads from {original_num_heads} to {self.num_heads}.")
        
        self.norm = nn.LayerNorm(dim)
        
        # 始终实例化 PyTorch 标准的多头注意力机制
        # 这是为了彻底绕开 flash-attn 的所有问题
        print(f"INFO: FlashAttnBlock (dim={dim}): Forcing use of torch.nn.MultiheadAttention to ensure stability.")
        self.attn = nn.MultiheadAttention(
            embed_dim=dim,
            num_heads=self.num_heads,
            dropout=dropout,
            batch_first=True  # 关键！因为我们会把数据 reshape 成 [B, S, D]
        )
        
    def forward(self, x, t=None, z=None):
        B, C, H, W = x.shape
        
        # 将输入从 [B, C, H, W] 转换为 [B, H*W, C] (序列形式)
        h = rearrange(x, "b c h w -> b (h w) c")
        h_norm = self.norm(h)
        
        # 直接使用 PyTorch 标准的多头注意力
        attn_output, _ = self.attn(h_norm, h_norm, h_norm, need_weights=False)
        
        # 将输出形状转换回 [B, C, H, W]
        attn_output = rearrange(attn_output, "b (h w) c -> b c h w", h=H, w=W)
