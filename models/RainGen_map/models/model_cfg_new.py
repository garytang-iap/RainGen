...
class EnhancedSparseUNet(nn.Module):
    def __init__(self, channels=1, condition_dim=7, nf=64, time_emb_dim=256, img_size=128, 
        ...
        # UNO坐标
        uno_coords = torch.stack(torch.meshgrid(
    torch.linspace(-1, 1, steps=uno_res),
    torch.linspace(-1, 1, steps=uno_res),
    indexing="ij"
))
    ...
    def forward(self, x, t, condition=None, sample_lst=None, coords=None, cond_drop_prob=None):
        """
        x:         [B, L, precip_channels]
        condition: [B, L, condition_dim]  (这里应为 7)
        sample_lst:[B, L]
        coords:    [B, L, 2]              (geo coords 给 UNO/grid_sample)
        """
        ...
        if (drop_prob > 0.0) and (condition is not None):
            if (cond_drop_prob is not None) or self.training:
                keep_mask = prob_mask_like((batch_size,), 1.0 - drop_prob, device=device).view(batch_size, 1, 1)

                null_cond = self.null_condition_embedding.to(device=device, dtype=condition.dtype)  # [1,1,D]
                null_cond = null_cond.expand(batch_size, condition.size(1), -1)                     # [B,L,D]

                condition = torch.where(keep_mask.expand_as(condition), condition, null_cond)
    ...
    indexing="ij"
        ...
        # 1. Down blocks (简化版，只有卷积，没有交叉注意力)
        downs = []
