...
import glob
# 【新增】开始: 添加用于读取预编码数据的数据集类
class PreprocessedNpzDataset(Dataset):
    """
    一个用于读取预编码 .npz 文件的 Pytorch 数据集。
    每个 .npz 文件应包含 'condition_latent' 和 'target_latent'。
    """
    def __init__(self, path):
        self.path = path
        self.files = sorted(glob.glob(os.path.join(self.path, "*.npz")))
        if not self.files:
            raise FileNotFoundError(f"No .npz files found in {self.path}")
        self.correction_factor = 1.0 / 9.0
    ...
    def __getitem__(self, idx):
        ...
        with np.load(filepath) as data:
            condition_latent = torch.from_numpy(data['condition_latent'].copy()).float()
            target_latent = torch.from_numpy(data['target_latent'].copy()).float()
            # --- 【核心修复】应用修正因子，将 latent 归一化到单位标准差 ---
        condition_latent = condition_latent * self.correction_factor
        target_latent = target_latent * self.correction_factor
        return condition_latent, target_latent
# (可选但推荐) 添加一个 collate_fn 来处理加载失败的样本
...
class LatentDataset(Dataset):
    """
    一个用于加载预编码好的 latent 数据的 PyTorch 数据集类。
    """
    def __init__(self, data_path, num_condition_frames=10, num_target_frames=30):
        """
        初始化数据集。
        
        Args:
            data_path (str): 存放预编码 .npz 文件的目录路径。
            num_condition_frames (int): 条件帧的数量。
            num_target_frames (int): 目标帧的数量。
        """
        super().__init__()
        self.data_path = data_path
        self.file_list = sorted(glob.glob(os.path.join(data_path, "*.npz")))
        self.num_condition_frames = num_condition_frames
        self.num_target_frames = num_target_frames
        
        if not self.file_list:
            raise FileNotFoundError(f"在路径 {data_path} 中没有找到任何 .npz 文件。")
    ...
    def __getitem__(self, idx):
        ...
        with np.load(file_path) as data:
            # 你的编码脚本保存的 key 是 'encoded_image' 和 'encoded_label'
            encoded_condition = data['encoded_image']
            encoded_future = data['encoded_label']

        # 转换为 PyTorch 张量
        # 你的编码脚本已经处理好了维度，输出是 (T, H, W, C_latent)
        # 我们需要把它变成 Latte 需要的 (T, C_latent, H, W)
        encoded_condition_tensor = torch.from_numpy(encoded_condition).permute(0, 3, 1, 2)
        encoded_future_tensor = torch.from_numpy(encoded_future).permute(0, 3, 1, 2)
        
        return encoded_condition_tensor, encoded_future_tensor

def prob_mask_like(shape, prob, device):
    """
    创建一个具有给定形状的布尔掩码。
    掩码中的每个元素以概率 `prob` 为 True。

    Args:
        shape (tuple): 掩码的形状。
        prob (float): 每个元素为 True 的概率。
        device (torch.device): 掩码所在的设备。
    ...
    if prob == 1:
        ...
        # 生成一个在 [0, 1) 区间内均匀分布的随机张量
        # 然后与概率 prob 进行比较，得到布尔掩码
...
class Latent_Dataset(Dataset):
    def __init__(self, data_dir, is_crop=True,size=160,input_length=10, label_length=30):
        """
        Args:
            data_dir (str): 数据目录路径
...
class MyNewDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            imgs=torch.tensor(np.load(os.path.join(self.data_dir,data_path))['image'],dtype=torch.float32).unsqueeze(1)
            

            lbs=torch.tensor(np.load(os.path.join(self.data_dir,data_path))['label'],dtype=torch.float32).unsqueeze(1)
...
def save_checkpoint(state, is_best_val, is_best_train=False, filename='checkpoint.pth.tar', best_model_path='model_best.pth.tar', train_best_path='train_best.pth.tar'):
    """
...
def plot_ensemble_spread(GT_data, all_members_data, ensemble_data, input_data, file_name, save_dir, time_idx=-1):
    ...
    plt.close()
def plot_long_forecast(GT_data, DDPM_data, input_data, save_dir=None):
    """
    为长时效预测（输入10帧，输出30帧）定制的绘图函数。
    它会展示输入、真值和预测在几个关键时间点的对比。
    """
    # --- 颜色和等级设置 ---
    levs = [5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0]
    cols = ["#D3D3D3","#01a0f6", "#00ecec", "#6dfa3d", "#00D806", "#019000", "#FFFF00", "#e7c000", "#FF9000", "#FF0000", "#d60000", "#C00000", "#e4007e", "#9600b4", "#AD90F0"]
    ...
    # --- 选择要绘制的时间点 ---
    # 输入: 10帧 (T-54min to T-6min)，我们选择5个点
    input_indices = [0, 2, 4, 6, 8]  # T-54, T-42, T-30, T-18, T-6 min
    # 输出: 30帧 (T+6min to T+180min)，我们选择5个关键预测点
    output_indices = [4, 9, 14, 19, 29] # T+30, T+60, T+90, T+120, T+180 min

    # --- 计算评估指标 ---
    mae_ddpm = MAE_loss(DDPM_data, GT_data)

    fig, ax = plt.subplots(3, 5, figsize=(25, 15))
    fig.suptitle('Radar Echo Long-term Forecast (10 frames -> 30 frames)', fontsize=24)

    # --- 绘制三行：真值、预测、输入 ---
    for i in range(5):
        # 绘制真值 (Ground Truth)
        gt_idx = output_indices[i]
        im = ax[0, i].imshow(GT_data[gt_idx], cmap=cmap, vmin=0.0, vmax=70.0)
        ax[0, i].set_title(f"Target at T+{(gt_idx + 1) * 6}min", fontsize=16)

        # 绘制预测 (Prediction)
        pred_idx = output_indices[i]
        ax[1, i].imshow(DDPM_data[pred_idx], cmap=cmap, vmin=0.0, vmax=70.0)
        ax[1, i].set_title(f"Prediction at T+{(pred_idx + 1) * 6}min", fontsize=16)
        # 添加指标文本
        csi_2 = CSI(DDPM_data[pred_idx], GT_data[pred_idx], 2.0)
        csi_8 = CSI(DDPM_data[pred_idx], GT_data[pred_idx], 8.0)
        ax[1, i].text(2, 10, f"MAE={mae_ddpm[pred_idx]:.2f}", color='white', backgroundcolor='black', fontsize=12)
        ax[1, i].text(2, 25, f"CSI(2)={csi_2:.2f}", color='white', backgroundcolor='black', fontsize=12)
        ax[1, i].text(2, 40, f"CSI(8)={csi_8:.2f}", color='white', backgroundcolor='black', fontsize=12)

        # 绘制输入 (Input)
        in_idx = input_indices[i]
        ax[2, i].imshow(input_data[in_idx], cmap=cmap, vmin=0.0, vmax=70.0)
        ax[2, i].set_title(f"Input at T-{(9 - in_idx) * 6}min", fontsize=16)

    ax[0, 0].set_ylabel("Ground Truth", fontsize=20)
    ax[1, 0].set_ylabel("Prediction", fontsize=20)
    ax[2, 0].set_ylabel("Input", fontsize=20)
    ...
    cbar = fig.colorbar(im, ax=ax.ravel().tolist(), orientation='vertical', shrink=0.8, pad=0.02)
    cbar.set_label('Radar Reflectivity (dBZ)', fontsize=16)
