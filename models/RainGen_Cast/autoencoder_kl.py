...
import numpy as np
class LatteConditionalEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Encodes input frames into latent space for the Latte model.
        
        Args:
            x (torch.Tensor): Input frames in shape [B, T, C_in, H, W].
                              Note: The input C_in is the physical channel (e.g., 1 for grayscale).
                              
        Returns:
            torch.Tensor: Latent representation in shape [B, T, C_latent, H', W'].
        """
        # 1. 检查输入维度
        if x.ndim != 5:
            raise ValueError(f"Expected a 5D tensor (B, T, C, H, W), but got {x.ndim}D.")
            
        b, t, c, h, w = x.shape
        
        # 2. Reshape for batch processing by VAE
        x_reshaped = rearrange(x, 'b t c h w -> (b t) c h w')
        
        # 3. Encode to get latent distribution
        posterior = self.vae_encoder.encode(x_reshaped)
        
        # 4. Get the mode (mean) of the distribution
        z = posterior.mode()
        
        # 5. Reshape back to include the time dimension
        z = rearrange(z, '(b t) c h w -> b t c h w', b=b, t=t)
        
        # 6. Apply scaling factor if provided
        if self.scale is not None:
            z = z / self.scale
            
        return z  # Final shape: [B, T, C_latent, H', W'], ready for Latte
class LatteVAEDecoder(nn.Module):
    def __init__(self, vae_decoder, scale=None):
        super().__init__()
        self.vae_decoder = vae_decoder
        self.scale = scale

    def forward(self, z: torch.Tensor,correction_factor) -> torch.Tensor:
        """
        Decodes latent representation from Latte back to the image space.
        
        Args:
            z (torch.Tensor): Latent representation from Latte in shape [B, T, C_latent, H', W'].
            
        Returns:
            torch.Tensor: Decoded frames in shape [B, T, C_out, H, W].
        """
        # 1. 检查输入维度
        if z.ndim != 5:
            raise ValueError(f"Expected a 5D latent tensor (B, T, C, H, W), but got {z.ndim}D.")
            
        b, t, c, h, w = z.shape

        # 1. 撤销我们在 Dataset 中做的放大操作
        z_corrected = z / correction_factor
        
        # 2. 应用 VAE 自身的 scale_factor (这是另一回事)
        if self.scale is not None:
            z_corrected = z_corrected * self.scale
        
        # 3. Reshape for batch processing by VAE decoder
        z_reshaped = rearrange(z_corrected, 'b t c h w -> (b t) c h w')
        
        # 4. Decode back to image space
        x_decoded = self.vae_decoder.decode(z_reshaped)
        
        # 5. Reshape back to include the time dimension
        x = rearrange(x_decoded, '(b t) c h w -> b t c h w', b=b, t=t)
        
        return x # Final shape: [B, T, C_out, H, W]
