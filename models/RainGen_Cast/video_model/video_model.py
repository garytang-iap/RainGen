...
from model.video_model.openaimodel import *
from model.video_model.video_attention import SpatialVideoTransformer
from model.video_model.spacetime_attention import (
    BasicTransformerTimeMixBlock,
    PostHocSpatialTransformerWithTimeMixing,
    PostHocSpatialTransformerWithTimeMixingAndMotion,
)

from model.video_model.util import AlphaBlender, get_alpha
...
class VideoUNet(nn.Module):
    def __init__(
        ...
        disable_temporal_crossattention: bool = True,
    ...
    def forward(
        ...
        image_only_indicator: Optional[th.Tensor] = None,
        precomputed_emb: Optional[th.Tensor] = None, # *** 新增参数 ***
    ):
        ...
        if precomputed_emb is not None:
            # 如果提供了预计算的 embedding，直接使用它
            emb = precomputed_emb
        else:
            # 否则，执行原来的离散时间步处理逻辑
            assert timesteps is not None, "Either timesteps or precomputed_emb must be provided."
