...
def convert_to_backend_form(x, sample_lst, img_size, backend="spconv", virtual_scale=1):
    """
    img_size: 物理分辨率 (196)
    virtual_scale: 放大倍数 (10)
    """
    # 1. 计算虚拟分辨率
    virtual_grid_size = int(img_size * virtual_scale)
    if backend == "spconv":
        # 2. 生成稀疏索引 (基于 1960x1960)
        sparse_indices = sample_lst_to_sparse_indices(sample_lst, virtual_grid_size)
        
        # 3. 构建 Tensor，声明超大空间
        x = SparseConvTensor(
            features=rearrange(x, "b l c -> (b l) c"),
            indices=sparse_indices.int(),
            spatial_shape=(virtual_grid_size, virtual_grid_size), 
            batch_size=x.size(0),
        )
        ...
        if virtual_scale > 1:
            raise ValueError("❌ Virtual High-Res Grid cannot use 'torch_dense'!")
        # 兼容旧代码...
...
def convert_to_backend_form_like(x, backend_tensor, sample_lst=None, img_size=None, backend="torch_dense", rearrange_x=True):
    if backend == "spconv":
        ...
        spatial_shape=backend_tensor.spatial_shape,   # ✅
        batch_size=backend_tensor.batch_size,
    )
    elif backend == "torchsparse":
        x = torchsparse.SparseTensor(
...
def sample_lst_to_sparse_indices(sample_lst, grid_size, ndims=2, dtype=torch.int32):
    """
    sample_lst: [B, L] 展平索引
    grid_size: int (虚拟大小，如 1960)
    """
    ...
    # 关键：使用 grid_size 解算行列
    sample_lst_h = sample_lst.div(grid_size, rounding_mode='trunc').to(dtype)
    sample_lst_w = (sample_lst % grid_size).to(dtype)
