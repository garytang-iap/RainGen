...
def _gather_from_grid(grid, sample_lst):
    B, C, H, W = grid.shape
    grid_flat = rearrange(grid, 'b c h w -> b c (h w)')
    indices = sample_lst.unsqueeze(1).expand(-1, C, -1)
    points = torch.gather(grid_flat, 2, indices)
    return rearrange(points, 'b c s -> b s c').contiguous()
...
def get_conv(dims, std, mode='reflect', channels=1):
...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.precip_channels = precip_channels # <--- 保存这个新参数
    ...
    def p_mean_variance(
        ...
        def process_xstart(x):
            if denoised_fn is not None:
                x = denoised_fn(x)
            if clip_denoised: # <--- clip_denoised=True 时，执行 clamp
                return x.clamp(-0.3, 11.5)
            return x
        # 接收的 x 是一个完整的网格, e.g., [B, 1, 32, 32]
        B, C, H, W = x.shape
        device = x.device
        # 1. 从 kwargs 中获取与 x 网格对齐的条件网格和坐标网格
        final_model_kwargs = model_kwargs.copy()
        condition_grid = final_model_kwargs.pop("condition_grid")
        coords_grid = final_model_kwargs.pop("coords_patch")

        # 2. 生成所有点的索引和坐标
        # 在推理时，我们总是处理所有点，所以 sample_lst 包含了 H*W 个索引
        sample_lst = torch.arange(H * W, device=device).repeat(B, 1)
        # 将坐标网格展平为散点坐标
        coords_points = _gather_from_grid(coords_grid, sample_lst)
        
        # 3. 将所有网格数据采样成散点
        x_t_points = _gather_from_grid(x, sample_lst)
        condition_points = _gather_from_grid(condition_grid, sample_lst)
        ...
        # 到此，所有准备工作完成。
        model_output_points = model(
    x_t_points,                           # 第一个位置参数：x
    self._scale_timesteps(t),             # 第二个位置参数：ts (timesteps)
    condition_points=condition_points,    # 关键字参数
    sample_lst=sample_lst,
    coords=coords_points,
    z=model_kwargs.get("z", None)
)
         # === 阶段三: 散点 -> 网格 (结果重构) ===
        # 将模型的散点输出重塑回网格，以便后续的扩散公式使用
        C_out = model_output_points.shape[-1]
        model_output = rearrange(model_output_points, "b (h w) c -> b c h w", h=H, w=W, c=C_out)
    ...
    def training_losses(self, model, x_start, t,  model_kwargs=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        coords_points = model_kwargs.pop("coords")
        sample_lst = model_kwargs.pop("sample_lst")
        noise = model_kwargs.pop("noise", None) # noise 也可以从 kwargs 获取
        encoder_z = model_kwargs.pop("z", None) # encoder 的 z 也可以从 kwargs 获取
        if noise is None:
            noise = torch.randn_like(x_start)
        ...
        if sample_lst is not None:
            # ======================= 在这里插入唯一的修改 =======================
            if model_kwargs is not None and "condition_grid" in model_kwargs:
                condition_grid = model_kwargs.pop("condition_grid")
                # 使用原始代码的逻辑来采样我们的条件
                condition_points = rearrange(condition_grid, 'b c h w -> b c (h w)')
                condition_points = torch.gather(condition_points, 2, sample_lst.unsqueeze(1).expand(-1, condition_grid.shape[1], -1)).contiguous()
                condition_points = rearrange(condition_points, 'b c s -> b s c')
                model_kwargs["condition_points"] = condition_points
