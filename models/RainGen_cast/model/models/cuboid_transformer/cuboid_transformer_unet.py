...
class CuboidTransformerUNet(nn.Module):
    ...
    def forward_with_cond_scale(
        self,
        x0,                 # 前四个时刻的降水场 [batch, latent_dim, frame=4, H, W]
        time,               # 时间步信息 [batch, ...]
        condition,          # 条件输入 [batch, latent_dim, in_frame=4, H, W]
        cond_scale=1.0,     # 条件缩放因子
        rescaled_phi=0.0,   # 可选的缩放参数
        remove_parallel_component=False,  # 是否移除并行成分
        keep_parallel_frac=0.0,           # 保持并行成分的比例
        **kwargs
    ):
        
        # 获取有条件的预测
        logits = self.forward(x0, time, condition)
